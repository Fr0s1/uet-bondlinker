...
func (r *PostRepo) Share(userID, postID uuid.UUID, content string) (*model.Post, error) {
	...
	if err := tx.Where("id = ?", postID).Scan(&originalPost).Error; err != nil {
		tx.Rollback()
		return nil, err
	}
	...
	var post model.Post
	if err := r.db.Preload("Author").Preload("SharedPost").Preload("SharedPost.Author").Where("id = ?", newPost.ID).Scan(&post).Error; err != nil {
		return nil, err
	}
